...
const input = cva(
  ...
  ],
  {
    variants: {
      variant: {
        default: [
          'border-2 border-gray-300 placeholder:text-[#9ca3af] h-9',
        ...
        subtle: [
          'bg-transparent border-none border-0 placeholder:text-[#9ca3af] h-9',
...
export type InputGlobalProps = React.HTMLAttributes<HTMLButtonElement> &
  VariantProps<typeof input> &
  React.ComponentPropsWithRef<'input'> &
  InputProps;

export const InputUI = forwardRef<HTMLInputElement, InputGlobalProps>(
  (
    {
      className,
      setParentValue,
